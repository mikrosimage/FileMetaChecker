...
BOOST_AUTO_TEST_CASE( element_checker_checker_groupSize )
{
	...
	LOG_INFO( "\n>>> element_checker_checker_groupSize <<<" );
	{
		...
		BOOST_CHECK_EQUAL( elem2->_warning.find( "Group size difference" ), std::string::npos );
	}
	LOG_INFO( "\n>>> element_checker_checker_groupSize invalid 1 <<<" );
	{
		const char buff1[4] { 0x00, 0x00, 0x00, 0x13 };
		spec_reader::Specification spec;
		spec.setFromString( jsonString );
		std::shared_ptr< spec_reader::SpecNode > node = spec.getFirstNode();
		BOOST_CHECK_EQUAL( node->getId(),                                       "value1"  );
		...
		BOOST_CHECK( elem6->next() == nullptr );
		BOOST_CHECK( elem2->_warning.find( "Group size difference" ) != std::string::npos );
	}
	LOG_INFO( "\n>>> element_checker_checker_groupSize invalid 2 <<<" );
	{
		const char buff1[4] { 0x00, 0x00, 0x00, 0x15 };
		spec_reader::Specification spec;
		spec.setFromString( jsonString );
		std::shared_ptr< spec_reader::SpecNode > node = spec.getFirstNode();
		BOOST_CHECK_EQUAL( node->getId(),                                       "value1"  );
